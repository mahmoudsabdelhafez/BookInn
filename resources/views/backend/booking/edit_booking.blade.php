                        ...
                        <div class="table-responsive">
                            ...
                            <div class="col-md-6" style="float: right">
                                ...
                                <table class="table test_table" style="float: right" border="none">
                                    ...
                                    @php 
                                    $subtotal = $editData->actual_price * $editData->number_of_rooms * $editData->total_night;
                                    // $discount = $editData->discount * $subtotal / 100;
                                    $grand_total = $subtotal - $editData->discount;
                                    @endphp
                                    <tr>
                                        <td>Subtotal</td>
                                        <td>${{ (int)$subtotal }}</td>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(".assign_room").on('click', function() {
                $.ajax({
                    url: "{{ route('assign_room', $editData->id) }}", // http request without reloading the page
                    success: function(data) { // The success callback is executed when the server responds successfully. // data is the data returned from the server 
                        $('.myModal .modal-body').html(data); // Finds the .modal-body inside the modal with the class .myModal.
                       // Replaces the content of .modal-body with the data received from the server.
                        $('.myModal').modal('show');
                    }
                });
                return false;
